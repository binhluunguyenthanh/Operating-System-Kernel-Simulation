...
int __alloc(struct pcb_t *caller, int vmaid, int rgid, addr_t size, addr_t *alloc_addr)
{
  ...
  cur_vma ->sbrk = old_sbrk + size; // add
...
int pg_getpage(struct mm_struct *mm, int pgn, int *fpn, struct pcb_t *caller)
{
  ...
  { /* Page is not online, make it actively living */
    ...
    // my implementation: get pte and fpn from victim page
    vicpte = pte_get_entry(caller, vicpgn);
    vicfpn = PAGING_FPN(vicpte);
    tgtfpn = vicfpn; // target frame will be the free frame
    ...
    regs.a2 = vicfpn; // Khung nguồn (Ram)
    regs.a3 = swpfpn; // Khung đích (SWAP)
    syscall(caller->krnl, caller ->pid, 17, &regs);
    // end my implementation
    /* Update page table */
    //pte_set_swap(...);
    pte_set_swap(caller, vicpgn, 0, swpfpn); //add
...
//    */
  ...
  regs.a2 = phyaddr; //memphy to read
  regs.a3 = (addr_t)data; //pointer to store data read
  syscall(caller->krnl, caller ->pid, 17, &regs);
  return 0;
}
...
int pg_setval(struct mm_struct *mm, int addr, BYTE value, struct pcb_t *caller)
{
  ...
  int phyaddr = (fpn << PAGING_ADDR_FPN_LOBIT) + off; // add variable
  /* TODO 
  ...
  regs.a2 = phyaddr; //memphy addr to write
  regs.a3 = (addr_t) value; //data need writing
  syscall(caller ->krnl, caller->pid, 17, &regs);
  return 0;
}
...
//  struct vm_area_struct *cur_vma = get_vma_by_num(caller->krnl->mm, vmaid);
  ...
  //my implementation check valid memory
...
int find_victim_page(struct mm_struct *mm, addr_t *retpgn)
{
  ...
  // prev->pg_next = NULL;
  //my implementation
  if (prev == NULL){ //there is only one node in list
    *retpgn = pg ->pgn;
    mm ->fifo_pgn = NULL;
  } else { // More than one
    *retpgn = pg ->pgn;
